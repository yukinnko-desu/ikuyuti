...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const startExitSequence = () => {
    // 200回到達後の爆発演出中に経過した時間を速度計測から除外
    const explosionEnterTime = localStorage.getItem("yuttinExplosionEnterTime");
    if (explosionEnterTime && firstTapAt) {
      const explosionDuration = Date.now() - parseInt(explosionEnterTime, 10);
      if (Number.isFinite(explosionDuration) && explosionDuration > 0) {
        firstTapAt += explosionDuration;
      }
      localStorage.removeItem("yuttinExplosionEnterTime");
    }
    persistResult();
    if (exitResultModal) {
  ...
  if (yearsImage) {
    ...
    yearsImage.addEventListener("click", () => {
      ...
      if (tapCount === 200) {
        // 爆発演出中の待ち時間を速度計測から除外するため、開始時刻を記録
        if (isMain && firstTapAt) {
          localStorage.setItem("yuttinExplosionEnterTime", String(Date.now()));
        }
